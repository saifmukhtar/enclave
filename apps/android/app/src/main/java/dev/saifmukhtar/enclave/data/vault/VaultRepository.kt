package dev.saifmukhtar.enclave.data.vault
...
import dev.saifmukhtar.enclave.crypto.ExifStripper
import dev.saifmukhtar.enclave.data.local.MediaMetadataDao
...
class VaultRepository(
    ...
    val bundleRepository: dev.saifmukhtar.enclave.network.BundleRepository
) {

    private fun getSharedVaultKey(): ByteArray? {
        val cryptoManager = dev.saifmukhtar.enclave.crypto.CryptoManager(context)
    ...
    suspend fun saveSecureFile(fileName: String, data: ByteArray) = withContext(Dispatchers.IO) {
        ...
        if (vaultKey != null) {
            try {
                val encrypted = dev.saifmukhtar.enclave.crypto.VaultCipher.encrypt(data, vaultKey)
    ...
    suspend fun readSecureFile(fileName: String): ByteArray = withContext(Dispatchers.IO) {
        ...
        if (vaultKey != null) {
            try {
                ...
                dev.saifmukhtar.enclave.crypto.VaultCipher.decrypt(encryptedBytes, vaultKey)
            } catch (e: Exception) {
