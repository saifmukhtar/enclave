package dev.saifmukhtar.enclave.notifications
...
import dev.saifmukhtar.enclave.data.local.MIGRATION_7_8
import dev.saifmukhtar.enclave.data.local.MIGRATION_8_9
import dev.saifmukhtar.enclave.data.local.MessageEntity
            ...
            withTimeoutOrNull<Unit>(5000) {
                coroutineScope {
                    ...
                    signalingClient.incomingRawMessages.collect { raw ->
                        try {
                            val msg = dev.saifmukhtar.enclave.webrtc.LenientJson.decodeFromString<dev.saifmukhtar.enclave.webrtc.SignalMessageWrapper>(raw)
                            if (msg.senderId != partnerId) return@collect
                            when (msg.type) {
                                "STORY_SHARE" -> {
                                    msg.payload?.let { payload ->
                                        try {
                                            val story = dev.saifmukhtar.enclave.webrtc.LenientJson.decodeFromString<dev.saifmukhtar.enclave.ui.profile.StorySharePayload>(payload)
                                            ...
                                            if (decryptedResult.isSuccess) {
                                                ...
                                                database.statusStoryDao().upsertStory(
                                                    dev.saifmukhtar.enclave.data.local.StatusStoryEntity(
                                                        id = story.storyId,
                                                        authorId = partnerId,
                        ...
                        if (baseType.startsWith("LOUNGE_")) {
                            if (baseType == "LOUNGE_NOTE_SYNC") {
                                try {
                                    ...
                                    val payloadObj = dev.saifmukhtar.enclave.webrtc.LenientJson.decodeFromString<dev.saifmukhtar.enclave.ui.lounge.SyncedNotePayload>(text)
                                    val entity = dev.saifmukhtar.enclave.data.local.EncryptedNoteEntity(
                                        id = payloadObj.id,
                                ...
                                try {
                                    val text = String(decryptedBytes, Charsets.UTF_8)
                                    val payloadObj = dev.saifmukhtar.enclave.webrtc.LenientJson.decodeFromString<dev.saifmukhtar.enclave.ui.lounge.SyncedLetterPayload>(text)
                                    val enc = cryptoManager.encryptLocal(payloadObj.plainContent.toByteArray(Charsets.UTF_8))
                                    val entity = dev.saifmukhtar.enclave.data.local.LetterEntity(
            ...
            try {
                if (partnerId.isNotBlank()) {
                    ...
                    if (partnerProfile != null) {
                        ...
                        // Force widget update
                        val intent = android.content.Intent(applicationContext, dev.saifmukhtar.enclave.ui.widget.CompanionWidgetProvider::class.java)
                        intent.action = android.appwidget.AppWidgetManager.ACTION_APPWIDGET_UPDATE
                        val ids = android.appwidget.AppWidgetManager.getInstance(applicationContext)
                            .getAppWidgetIds(android.content.ComponentName(applicationContext, dev.saifmukhtar.enclave.ui.widget.CompanionWidgetProvider::class.java))
