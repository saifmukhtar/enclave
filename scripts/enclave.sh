...
readonly REPO_SPARSE_PATH="backend/server"   # Only this subfolder is cloned
...
echo -e "      ${DIM}Uses Git sparse-checkout — downloads backend/server/ only (not the Android app)${RESET}"
echo ""
...
# ─────────────────────────────────────────────────────────────────
#  STEP 2 — SPARSE CLONE (backend/server ONLY)
# ─────────────────────────────────────────────────────────────────
header "STEP 2  ·  Cloning backend/server from GitHub"
echo ""
...
if [[ "${_do_clone:-false}" == "true" ]]; then
  step "Cloning backend/server/ from ${REPO_URL} (branch: ${REPO_BRANCH})..."
  git clone \
    --filter=blob:none \
    --sparse \
    --branch "${REPO_BRANCH}" \
  ...
  # Flatten: move backend/server/* up to INSTALL_DIR if it's nested
